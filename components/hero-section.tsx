...
export function HeroSection() {
  ...
  return (
    <section ref={sectionRef} id="hero" className="relative min-h-screen flex items-center pt-20 md:pt-0 pl-6 md:pl-28 pr-6 md:pr-12">
      ...
      <motion.div
        className="flex-1 w-full relative z-10 py-20"
      ...
      >
        ...
        <SplitFlapAudioProvider>
          <div className="relative inline-flex flex-col gap-2 p-4 sm:p-6 md:p-10 mb-6 group w-[calc(100vw-3rem)] sm:w-auto overflow-hidden sm:overflow-visible">
            {/* Cinematic Camera Viewfinder HUD */}
            <div className="absolute inset-0 pointer-events-none overflow-hidden sm:overflow-visible">
              {/* Box border */}
              <div className="absolute inset-2 sm:inset-4 border border-foreground/10 transition-colors duration-700 group-hover:border-accent/30" />
              ...
              {/* Center crosshair */}
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 opacity-20 transition-all duration-700 group-hover:opacity-40 group-hover:scale-110">
        ...
        <h2 className="font-[var(--font-bebas)] text-muted-foreground/80 md:text-foreground/90 text-[clamp(1.5rem,4vw,3rem)] mt-6 tracking-wider bg-clip-text text-transparent bg-gradient-to-r from-foreground to-muted-foreground">
          Visual Storytelling & Event Media Production
        </h2>

        <p className="mt-8 max-w-xl font-mono text-sm md:text-base text-muted-foreground leading-relaxed border-l-2 border-accent/40 pl-6">
        ...
        <div className="mt-16 flex flex-col sm:flex-row items-start sm:items-center gap-6 sm:gap-10">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="group relative inline-flex items-center gap-4 bg-accent/10 border border-accent/40 px-8 py-4 font-mono text-xs uppercase tracking-widest text-foreground hover:bg-accent hover:text-accent-foreground hover:border-accent shadow-[0_0_20px_rgba(var(--accent-rgb),0.2)] hover:shadow-[0_0_30px_rgba(var(--accent-rgb),0.5)] transition-all duration-400 ease-out"
          >
          ...
          <a
            href="#work"
            className="group flex items-center gap-3 font-mono text-xs uppercase tracking-widest text-muted-foreground hover:text-foreground transition-colors duration-200"
          >
      ...
      <div className="absolute bottom-8 right-8 md:bottom-12 md:right-12 z-20 flex flex-col items-end gap-3">
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="border border-border/50 px-4 py-2 font-mono text-[10px] uppercase tracking-widest text-muted-foreground hover:text-accent hover:border-accent/50 transition-colors bg-background/50 backdrop-blur-sm"
        >
          Follow on Instagram
        </a>
        <div className="border border-emerald-500/30 px-4 py-2 font-mono text-[10px] uppercase tracking-widest text-emerald-400 font-bold bg-emerald-500/10 backdrop-blur-sm relative overflow-hidden">
